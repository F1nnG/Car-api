	...
	<div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
		...
		<div class="relative p-4 rounded-lg shadow bg-gray-800 sm:p-5">
			...
			<div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 border-gray-600">
				...
				<a href="{{ route('admin.cars') }}" class="text-gray-400 bg-transparent rounded-lg text-sm p-1.5 ml-auto inline-flex items-center hover:bg-gray-600 hover:text-white">
			...
			<form action="{{ route('admin.cars.update', ['car' => $car->id]) }}" method="POST">
				...
				<div class="grid gap-4 mb-4 sm:grid-cols-2">
					<div class="sm:col-span-2">
						...
						<input type="text" name="model" id="model" class="border text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-primary-500 focus:border-primary-500" placeholder="Type model name" value="{{ $car->model }}" required>
					</div>
					<div>
						<label for="price" class="block mb-2 text-sm font-medium text-white">Price</label>
						<input type="number" name="price" id="price" class="border text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-primary-500 focus:border-primary-500" placeholder="Car price" value="{{ $car->price }}" required>
					</div>
					<div>
					...
					<div>
						...
						<input type="number" name="usage" id="usage" class="border text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-primary-500 focus:border-primary-500" placeholder="Car usage" value="{{ $car->usage }}" required>
					</div>
				</div>
