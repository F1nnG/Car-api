		...
		<div class="bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
			<div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
				...
				<div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
					<button type="button" data-modal-toggle="create-car-modal" class="flex items-center justify-center text-white focus:ring-4 font-medium rounded-lg text-sm px-4 py-2 bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-primary-800">
					...
					<tbody>
						@foreach ($cars as $car)
							<tr class="border-b border-gray-700">
								<th scope="row" class="px-4 py-3 font-medium whitespace-nowrap text-white">{{ $car->brand->name . ' ' . $car->model }}</th>
								<td class="px-4 py-3">{{ $car->price }}</td>
								<td class="px-4 py-3">{{ $car->type }}</td>
								<td class="px-4 py-3">{{ $car->usage }}</td>
								<td class="px-4 py-3 flex items-center justify-end">
									<button id="action-dropdown-{{ $car->id }}-button" data-dropdown-toggle="action-dropdown-{{ $car->id }}" class="inline-flex items-center p-0.5 text-sm font-medium text-center rounded-lg focus:outline-none text-gray-400 hover:text-gray-100" type="button">
										<svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
											<path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
										</svg>
									</button>
									<div id="action-dropdown-{{ $car->id }}" class="hidden z-10 w-44 rounded divide-y shadow bg-gray-700 divide-gray-600">
										<ul class="py-1 text-sm text-gray-200" aria-labelledby="action-dropdown-{{ $car->id }}-button">
											<li>
												<a href="{{ route('admin.cars', ['edit' => $car->id]) }}" class="block py-2 px-4 hover:bg-gray-600 hover:text-white">Edit</a>
											</li>
										</ul>
										<div class="py-1">
